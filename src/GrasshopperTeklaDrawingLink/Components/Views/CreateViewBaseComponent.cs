...
namespace GTDrawingLink.Components.Views
{
    ...
    public abstract class CreateViewBaseComponentNew<T> : CreateDatabaseObjectComponentBaseNew<T> where T : CommandBase, new()
    {
        protected bool _loadAttributesWithMacro;

        protected CreateViewBaseComponentNew(GH_InstanceDescription info) : base(info)
        {
            SetCustomMessage();
        }
