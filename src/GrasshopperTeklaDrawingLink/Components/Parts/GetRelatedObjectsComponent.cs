        ...
        protected override void InvokeCommand(IGH_DataAccess DA)
        {
            ...
            for (int i = 0; i < inputTree.Count; i++)
            {
                ...
                for (int j = 0; j < branchObjects.Count; j++)
                {
                    ...
                    {
                        var typePath = newPath.AppendElement(k);
                        var objectGoos = relatedObjectsGroupedByType[k].Select(o => new TeklaDatabaseObjectGoo(o));
                        outputTree.AppendRange(objectGoos, typePath);
                        outputKeys.Append(new GH_String(relatedObjectsGroupedByType[k].Key), typePath);
                    }
                }
            }
